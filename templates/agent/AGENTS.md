...
9. Check `00-Inbox/` — note item count, flag if > 20
10. Check if `BOOTSTRAP.md` exists — if yes, follow it instead of this
11. **Detect planning context** — see Planning Conversation Protocol below
12. Then respond with the appropriate greeting (see step 11)

> **Fast start:** If CONTEXT-PACK.md exists and was updated today, read only steps 1, 5, 6, 7, 8, 9, 11 — then respond. Full reads needed only when context pack is stale (>24h old).
> **Note:** Steps 7-8 (specialist briefings + vault health) are ALWAYS read, even on fast start. Briefings are short and contain time-sensitive context.
