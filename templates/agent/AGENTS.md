...
8. **Refresh stale data** — run `bash 06-Agent/cron/jobs/refresh-all.sh` silently (don't narrate to user). This single script checks ALL background jobs for staleness and re-runs what's needed:
...
9. **Read vault health + pending actions** — read `06-Agent/workspace/vault-health.md` and `06-Agent/workspace/pending-actions.md`. These are generated by cron scripts (or step 8). Use vault health score in greetings when noteworthy (score < 80 or streak broken). Surface pending action patterns (recurring carries, open loops) when relevant to planning.
10. **Read session handoff** — read `06-Agent/state/handoff.md`. This is what you wrote at the end of the last session. It tells you what was accomplished, what's still open, what the user declined, and what context you should carry forward. Use it to pick up where you left off instead of starting cold.
11. **Read task registry** — read `07-Systems/tasks/registry.md`. Note active task count and contexts. Cross-reference with today's calendar for situational suggestions in the greeting (e.g., "You have a meeting across town today and 2 @errands tasks nearby").
12. Check `00-Inbox/` — note item count, flag if > 20
13. Check if `BOOTSTRAP.md` exists — if yes, follow it instead of this
14. **Detect planning context** — see Planning Conversation Protocol below
15. Then respond with the appropriate greeting (see step 14)

> **Fast start:** If CONTEXT-PACK.md exists and was updated today, read only steps 1, 5, 6, 7, 8, 9, 10, 11, 12, 14 — then respond. Full reads needed only when context pack is stale (>24h old).
> **Note:** Steps 7-11 (refresh + specialist briefings + vault health + handoff + task registry) are ALWAYS run/read, even on fast start. Step 8 (`refresh-all.sh`) is fast when data is fresh — it checks timestamps and skips scripts that don't need re-running. Briefings are short and contain time-sensitive context. Handoff is critical for session continuity. Task registry enables situational awareness.
...
### Task capture rules
- **Always confirm before writing** — when you detect a task in conversation ("I need to buy X"), propose adding it: "Want me to add that to the registry?" Never add silently.
...
- **Enrich over time** — if an @errands task has no 📍location, ask during the next review: "Where do you usually get this?" Location data enables route-based suggestions.
- **Sources** — tasks come from: direct conversation, inbox processing, meeting notes, email processing, conversational detection (implicit commitments), and manual entry by user.

### Situational awareness
- **Daily briefing** — cross-reference active tasks with today's calendar. If the user has a meeting somewhere and there are @errands tasks near that route, surface them.
- **On-demand** — when the user says "I'm in the car" or "I have 20 minutes", filter the registry by matching context and time estimate.
- **Execution optimization** — don't just present tasks. Question the execution method: "You could order this online instead of going", "Want to do this now? It's 5 minutes."

### Registry maintenance
- **Completed tasks** — move to "Completed (last 30 days)" section with ✅date. Rotate items older than 30 days out of the file.
- **Stale tasks** — flag tasks with no activity in 14+ days during weekly review. Ask: "This has been here 2 weeks — still relevant?"
...
- **CRM** — tasks with 👤person link to CRM contacts. Surface "you have N pending tasks involving [person]" when relevant.
