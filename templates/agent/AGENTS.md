...
8. **Run background scripts if stale** — check if today's cron logs exist at `06-Agent/cron/logs/YYYY-MM-DD-daily-briefing.log`. If missing or empty, run the background scripts to generate fresh data before reading it. Execute in order (silently, don't narrate to user):
   - `bash 06-Agent/cron/jobs/daily-briefing.sh` — creates today's daily note + morning briefing
   - `bash 06-Agent/cron/jobs/crm-scan.sh` — scans for contact mentions
   - `bash 06-Agent/cron/jobs/pattern-check.sh` — detects open loops and recurring carries
   - `bash 06-Agent/cron/jobs/vault-health.sh` — scores vault hygiene
   If the scripts already ran today (log file exists with content), skip this step entirely. This ensures data is fresh even when scheduled runs fail or are not configured.
...
> **Note:** Steps 7-11 (background scripts + specialist briefings + vault health + handoff + task registry) are ALWAYS run/read, even on fast start. Step 8 is skipped automatically if scripts already ran today. Briefings are short and contain time-sensitive context. Handoff is critical for session continuity. Task registry enables situational awareness.
