...
10. **Read task registry** — read `07-Systems/tasks/registry.md`. Note active task count and contexts. Cross-reference with today's calendar for situational suggestions in the greeting (e.g., "You have a meeting across town today and 2 @errands tasks nearby").
11. Check `00-Inbox/` — note item count, flag if > 20
12. Check if `BOOTSTRAP.md` exists — if yes, follow it instead of this
13. **Detect planning context** — see Planning Conversation Protocol below
14. Then respond with the appropriate greeting (see step 13)

> **Fast start:** If CONTEXT-PACK.md exists and was updated today, read only steps 1, 5, 6, 7, 8, 9, 10, 11, 13 — then respond. Full reads needed only when context pack is stale (>24h old).
> **Note:** Steps 7-10 (specialist briefings + vault health + handoff + task registry) are ALWAYS read, even on fast start. Briefings are short and contain time-sensitive context. Handoff is critical for session continuity. Task registry enables situational awareness.
